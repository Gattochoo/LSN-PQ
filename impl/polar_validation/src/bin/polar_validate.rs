...
fn main() {
    ...
    let mut output = PathBuf::from("experiments/123--polar-rust-baseline.json");
...
.expect("--list-size must be a positive integer");
    ...
    let (results, decoder_label, experiment) = match decoder.as_str() {
        ...
        "scl" => (
            ...
            format!("-p1-rust-scl-l{list_size}-{suite}"),
),
        "scl-fast" => (
            run_and_report(&configs, |cfg| {
            ...
            format!("-p1-rust-scl-fast-l{list_size}-{suite}"),
),
        "fixed-i64" => (
            run_and_report(&configs, |cfg| {
                assert_eq!(
            ...
            format!("-p1-rust-scl-fixed-i64-l{list_size}-{suite}"),
),
...
fn print_help() {
    println!(
        "polar-validate [--trials N] [--seed U64] [--output PATH]\n\
         ...
         [--check PATH]\n\
         Runs a  P1 Rust polar validation suite."
);
}
