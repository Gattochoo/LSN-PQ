...
fn result_json_records_threat_model_and_success_rate() {
    ...
    let json = results_to_json("-p2-ml-smoke", &[result]);

    assert!(json.contains("\"experiment\": \"-p2-ml-smoke\""));
...
fn sampled_candidate_ml_json_records_elapsed_ms_when_available() {
    ...
    let json = sampled_candidate_ml_results_to_json("-p2-elapsed-smoke", &[result]);
...
fn sampled_candidate_ml_json_records_strict_and_tie_success_counts() {
    ...
    let json = sampled_candidate_ml_results_to_json("-p2-tie-smoke", &[result]);
...
fn span_result_json_records_overfull_failure_stats() {
    ...
    let json = span_results_to_json("-p2-span-smoke", &[result]);
...
#[test]
fn streaming_candidate_budget_runner_matches_stored_runner() {
    let stored =
        run_sampled_candidate_ml_budget_trials(4, 96, 0.25, 3, &[16, 64, 128], 0x517E_ADED_BA5E);
    let streaming = run_sampled_candidate_ml_budget_trials_streaming(
        4,
        96,
        0.25,
        3,
    ...
    assert_eq!(streaming, stored);
}

#[test]
fn streaming_ambient_runner_matches_stored_runner() {
    let stored = run_sampled_candidate_ambient_ml_trials(4, &[0.25], &[0.25], 2, 0xA11_AAAA);
    let streaming =
        run_sampled_candidate_ambient_ml_trials_streaming(4, &[0.25], &[0.25], 2, 0xA11_AAAA);
