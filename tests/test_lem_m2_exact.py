...
from experiments.lib import lem_m2_exact as m2


def test_uniform_b_matched_rate_axis_constants():
    """Uniform-B lem:m2 comparisons must use LPN_{p_eff(n)}, not LPN_{1/4}."""
    assert m2.uniform_b_effective_noise_rate(2) == Fraction(175, 512)
    assert m2.uniform_b_effective_noise_rate(3) == Fraction(3367, 8192)
    assert m2.uniform_b_graph_mass(2) == Fraction(29, 64)
    assert m2.uniform_b_graph_mass(3) == Fraction(1241, 4608)
    assert m2.uniform_b_effective_noise_rate(2) != Fraction(1, 4)
